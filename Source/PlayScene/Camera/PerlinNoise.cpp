...
namespace PerlinNoise
{
	...
	//const float ratioX = smootherstep(x - floor(x));
	//const float ratioY = smootherstep(x - floor(x));

}

VECTOR3 PerlinNoise::noise(VECTOR3 p)
{
	// (x, y)を囲む４つの格子点
	VECTOR3 tl = VECTOR3(floor(p.x), floor(p.y), floor(p.z));
	return VECTOR3();
}

float PerlinNoise::smootherstep(float x) {
	return x * x * x * (x * (x * 6 - 15) + 10);
}

float PerlinNoise::dot(VECTOR3 a, VECTOR3 b)
{
